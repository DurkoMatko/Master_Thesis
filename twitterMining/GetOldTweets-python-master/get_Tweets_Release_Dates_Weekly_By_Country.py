...
countries = [ "United States", "Japan","United Kingdom","India", "Pakistan", "Brazil", "Austria"]
	...
	for country in countries:
		...
		with open(os.path.join(os.path.dirname(__file__), 'releaseDates/') + oneFile) as releaseDatesFile:
			...
			for releaseDate in searchDates:
				...
				miningConsoleCommand = "python Exporter.py --querysearch '" + frameworkName + "' --since " + str(fromDate) + " --until " + str(toDate)  + " --lang " + "en" + " --maxtweets " + str(TWEETS_PER_RELEASE) + " --near '" + country + "' --output='" + frameworkName + '_' + country + ".csv" + "'"
				print miningConsoleCommand
				#execute the command
