...
for oneFile in releaseDatefiles:
	...
	with open(os.path.join(os.path.dirname(__file__), 'releaseDates/') + oneFile) as releaseDatesFile:
		...
		for line in content:
			...
			#calculate bounds
			beforeRelease_from = releaseDate + datetime.timedelta(days=4)
			beforeRelease_to = releaseDate + datetime.timedelta(days=1)
			...
			print beforeRelease_from;
			miningConsoleCommand = "python Exporter.py --querysearch '" + frameworkName + "' --since " + str(beforeRelease_to) + " --until " + str(beforeRelease_from) + " --output='" + frameworkName + "_after.csv" + "'"
